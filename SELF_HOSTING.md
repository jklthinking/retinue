...
`retinue-data` volume. It starts the authenticated v0.2 hub (not the old
read-only panel). Set an admin password of at least eight characters, then
start:
...
`GET /api/health` is unauthenticated and reports `status=ok` once the process
is up. The first start runs `migrate` and creates the admin when the
password is set; later starts skip a duplicate account.
...
`127.0.0.1:9219:9219` on the host. The file-mode panel (`retinue panel`)
still defaults to loopback port 8787 and the same publication rule:
