...
PIPELINE = [
    ...
    {"name": "审批门", "holder": "boss", "gate": "queen"},
]
...
def test_full_pipeline_with_queen_gate(env):
    ...
    assert len(pending) == 1 and pending[0]["stage_name"] == "审批门"
    ...
    response = client.post(
        ...
        json={
            ...
            "pipeline": [
                ...
                {"name": "伪审批门", "holder": "checker", "gate": "queen"},
            ],
        },
    )
...
def test_rejecting_to_queen_gate_opens_fresh_approval(env):
    ...
    created = client.post("/api/tasks", json={"title": "退回审批门", "pipeline": pipeline})
