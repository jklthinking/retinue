...
# One atomic statement both creates the counter row (seeded from rows that
# predate the counter, e.g. imported cards) and advances it. Running inside
# the caller's transaction keeps allocation rollback-safe: an aborted change
# withdraws its sequence value, so concurrent writers see neither a duplicate
# nor a gap. The CASE guard keeps the counter monotonic when a row was
# inserted out of band (legacy import) after the counter came into being.
_ALLOCATE_SQL = text(
    "INSERT INTO seq_counters (name, value) VALUES (:name, :seed)"
    " ON CONFLICT(name) DO UPDATE SET value ="
    " CASE WHEN seq_counters.value >= excluded.value"
    " THEN seq_counters.value + 1 ELSE excluded.value END"
)


def _allocate_sequence(db: Session, name: str, current_max: int) -> int:
    """Atomically advance the named counter past ``current_max`` and return it."""
    db.execute(_ALLOCATE_SQL, {"name": name, "seed": current_max + 1})
    return db.execute(
        select(SeqCounter.value).where(SeqCounter.name == name)
    ).scalar_one()
...
def next_task_id(db: Session) -> str:
    ...
    serial = _allocate_sequence(db, f"task-id:{today}", current_max)
    if serial > 999:
        raise ProtocolError("daily task id space exhausted (999 cards)")
    return f"{prefix}{serial:03d}"


def _next_event_seq(db: Session, task_id: str) -> int:
    current_max = db.execute(
        select(func.max(TaskEvent.seq)).where(TaskEvent.task_id == task_id)
    ).scalar()
    return _allocate_sequence(db, f"task-events:{task_id}", current_max or 0)


def _next_attempt_seq(db: Session, task_id: str) -> int:
    current_max = db.execute(
        select(func.max(TaskAttempt.seq)).where(TaskAttempt.task_id == task_id)
    ).scalar()
    return _allocate_sequence(db, f"task-attempts:{task_id}", current_max or 0)
...
def append_attempt(
    ...
    *,
    ...
    attempt = TaskAttempt(
        task_id=task.id,
        seq=_next_attempt_seq(db, task.id),
        attempt_key=key,
...
def _append_review_event(
    ...
    *,
    ...
    db.refresh(task)
    event = TaskEvent(
        seq=_next_event_seq(db, task.id),
        who=who,
...
def append_annotation_event(
    ...
    *,
    ...
    """Append a non-state chain event (mention, squad route, trigger notice)."""
    note = did.strip()
    if not note:
        raise ProtocolError("annotation note must be non-empty")
    if len(note) > 240:
        note = note[:240]
    canonical = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    if event_key:
...
def _append_dependency_event(db: Session, task: Task, *, who: str, note: str) -> None:
...
def add_task_dependency(
    ...
    *,
    ...
    db.add(edge)
    if append_event:
        _append_dependency_event(db, task, who=who, note=note)
    db.flush()
    return edge, True
...
def create_task(
    ...
    *,
    ...
    normalized_squad = squad_id.strip() if squad_id and squad_id.strip() else None
    if normalized_squad:
        from .db import Squad as SquadRow

        if db.get(SquadRow, normalized_squad) is None:
            raise ProtocolError(f"unknown squad: {normalized_squad}")
        if not open_dispatch:
            raise ProtocolError("a squad address requires an open-dispatch card")
    ...
    if normalized_squad:
        initial_payload["squad_id"] = normalized_squad
    if event_payload:
...
def update_task(
    ...
    *,
    ...
    db.flush()
    if new_status == "done" and state_changed:
        try:
            from .qc_hook import maybe_notify_task_done

            maybe_notify_task_done(task)
        except Exception:
            logging.getLogger(__name__).exception(
                "qc_hook notify failed after %s reached done", task.id
            )
    return task
