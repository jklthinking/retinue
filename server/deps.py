...
from .db import ApiToken, ChannelToken, Node, NodeToken, User, WebSession, utcnow
...
@dataclass
class Principal:
    kind: str  # "user" | "agent" | "channel"
    name: str  # username, actor id, or channel id
    actor_id: str | None
...
def authenticate(request: Request, db: Session) -> Principal | None:
    ...
    if header.lower().startswith("bearer "):
        ...
        if record is not None:
            ...
            return None
        channel = db.execute(
            select(ChannelToken).where(ChannelToken.token_hash == hash_token(token))
        ).scalar()
...
def require_auth(request: Request, db: Session = Depends(get_db, scope="function")) -> Principal:
    ...
    if principal.role == "channel" and not channel_request_allowed(request):
        raise HTTPException(
            status_code=403,
            detail="通道令牌只能开卡与读自己开的卡，不能冒充执行者",
        )
    return principal


# A channel credential has exactly two capabilities: open a card through the
# existing create route, and read the cards its own channel opened. Every
# other route — claim, notes, dispatch, admin — stays executor or operator
# territory, so the gate is centralized here instead of sprinkled per route.
_CHANNEL_CREATE_PATH = "/api/tasks"
_CHANNEL_READ_PATTERN = re.compile(r"^/api/tasks(/task-[0-9]{8}-[0-9]{3})?$")


def channel_request_allowed(request: Request) -> bool:
    path = request.url.path
    if request.method == "POST" and path == _CHANNEL_CREATE_PATH:
        return True
    if request.method in {"GET", "HEAD", "OPTIONS"}:
        return bool(_CHANNEL_READ_PATTERN.match(path))
    return False
...
def require_channel(
    ...
    """Authenticate an entry-channel credential for the intake webhook route."""
    principal = authenticate(request, db)
    if principal is None:
        raise HTTPException(status_code=401, detail="authentication required")
    if principal.kind != "channel":
        raise HTTPException(
            status_code=403, detail="channel-scoped credential required"
        )
    return principal
