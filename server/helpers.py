...
def notify_feishu(text: str) -> None:
    """Fire-and-forget custom-bot webhook; off unless RETINUE_FEISHU_WEBHOOK is set."""
    webhook = os.environ.get("RETINUE_FEISHU_WEBHOOK", "").strip()
    if not webhook:
        return
    ...
    def _send() -> None:
        ...
        try:
            open_url(request, timeout=10, request_class=RequestClass.OUTWARD).close()
        except OSError:
            pass  # notifications must never break the task flow
...
def build_orientation_context(db: Session, principal: Principal) -> dict[str, Any]:
    ...
    rules = [
        ...
        "交棒、打回、审核和女王决策都写入同一条任务链，不复制第二份状态。",
