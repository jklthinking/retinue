...
from ..db import Actor, ApiToken, ChannelToken, ChannelUser, Node, NodeToken, User, utcnow
...
# ---------- intake channels (schema v16): channel credentials and the
# channel-user → board-actor mapping table ----------


@router.post("/api/admin/channel-tokens")
def admin_create_channel_token(
    body: ChannelTokenBody,
    principal: Principal = Depends(require_admin),
    db: Session = Depends(get_db, scope="function"),
) -> dict[str, Any]:
    token = new_token("rtc")
...
@router.get("/api/admin/channel-tokens")
def admin_channel_tokens(
    principal: Principal = Depends(require_admin),
    db: Session = Depends(get_db, scope="function"),
) -> list[dict[str, Any]]:
    return [
        ...
        for t in db.execute(
            select(ChannelToken).order_by(ChannelToken.created_at)
        ).scalars()
    ]


@router.post("/api/admin/channel-tokens/{token_id}/revoke")
def admin_revoke_channel_token(
    token_id: int,
    principal: Principal = Depends(require_admin),
    db: Session = Depends(get_db, scope="function"),
) -> dict[str, Any]:
    token = db.get(ChannelToken, token_id)
    if token is None:
        raise HTTPException(status_code=404, detail=f"unknown channel token: {token_id}")
    if token.disabled:
        raise HTTPException(
            status_code=409, detail=f"channel token is already revoked: {token_id}"
        )
    token.disabled = True
    return {"status": "revoked", "id": token.id, "channel_id": token.channel_id}


@router.get("/api/admin/channel-users")
def admin_channel_users(
    channel_id: str | None = None,
    principal: Principal = Depends(require_admin),
    db: Session = Depends(get_db, scope="function"),
) -> list[dict[str, Any]]:
    query = select(ChannelUser).order_by(ChannelUser.created_at)
    if channel_id:
        query = query.where(ChannelUser.channel_id == channel_id)
    return [
        {
            "id": m.id,
        ...
        for m in db.execute(query).scalars()
    ]


@router.put("/api/admin/channel-users")
def admin_upsert_channel_user(
    body: ChannelUserBody,
    principal: Principal = Depends(require_admin),
    db: Session = Depends(get_db, scope="function"),
) -> dict[str, Any]:
    """Bind one channel-internal user identity to one board actor (upsert)."""
    if db.get(Actor, body.actor_id) is None:
        raise HTTPException(status_code=422, detail=f"unknown actor: {body.actor_id}")
    mapping = db.execute(
        select(ChannelUser).where(
            ChannelUser.channel_id == body.channel_id,
            ChannelUser.channel_user_id == body.channel_user_id,
        )
    ).scalar()
    if mapping is None:
        mapping = ChannelUser(
        ...
        mapping.actor_id = body.actor_id
...
@router.delete("/api/admin/channel-users/{mapping_id}")
def admin_delete_channel_user(
    mapping_id: int,
    principal: Principal = Depends(require_admin),
    db: Session = Depends(get_db, scope="function"),
) -> dict[str, Any]:
    mapping = db.get(ChannelUser, mapping_id)
    if mapping is None:
        raise HTTPException(status_code=404, detail=f"unknown mapping: {mapping_id}")
    db.delete(mapping)
