...
def stage_done(
    ...
    *,
    ...
    if stages[index]["gate"] == "queen":
        raise ProtocolError("人工审批节点由审批裁决,不能 stage-done")
...
def decide_approval(
    ...
    *,
    ...
    stage = stages[index]
    if stage["gate"] != "queen":
        raise ProtocolError("审批不再指向人工审批节点")
    ...
    if decision == "approve":
        ...
        if next_index >= len(stages):
            update_task(
                ...
                note=f"审批门「{stage_name}」批准{suffix}",
                flow_driven=True,
            )
            update_task(
        ...
        entered = _enter_stage(
            ...
            note=f"审批门「{stage_name}」批准,交棒「{stages[next_index]['name']}」{suffix}",
        )
    ...
    entered = _enter_stage(
        ...
        note=f"审批门「{stage_name}」驳回,退回「{stages[prev]['name']}」{suffix}",
    )
    fresh, tokens = entered if entered is not None else (None, None)
    return {
