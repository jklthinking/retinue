...
def skill_sync_digest(
    *,
    ...
    """Canonical hash for node skill sync idempotency (name + fields + snapshot)."""
...
def upsert_synced_skill(
    ...
    *,
...
) -> tuple[Skill, Literal["created", "updated", "unchanged"]]:
    """Idempotent catalog upsert for POST /api/skills/sync; M0 never deletes."""
    existing = db.execute(select(Skill).where(Skill.name == name)).scalar()
    incoming = skill_sync_digest(
        name=name,
    ...
    if existing is not None:
        try:
            stored_snap = json.loads(existing.source_snapshot_json or "{}")
        except json.JSONDecodeError:
            stored_snap = {}
        if not isinstance(stored_snap, dict):
            stored_snap = {}
        stored = skill_sync_digest(
            name=existing.name,
            description=existing.description or "",
            category=existing.category or "",
            snapshot=stored_snap,
        )
        if stored == incoming:
            return existing, "unchanged"
        status: Literal["created", "updated", "unchanged"] = "updated"
    else:
        status = "created"
