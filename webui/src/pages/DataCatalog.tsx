...
export default function DataCatalog() {
  ...
  if (!data && !error) return <div className="rt-loading"><Database size={25} />正在读取众卿数据目录…</div>;
  ...
  return (
    <div className="rt-page data-catalog-page">
      ...
      {section === "layers" && <>
        <Panel icon={<Database size={15} />} kicker="STORAGE LAYERS" title="组织数据层" tools={<span className="data-catalog-panel-note">只展示目录与字段，不读取原始正文</span>}>
          ...
          <div className="data-catalog-boundary"><LockKeyhole size={15} /><span>组织中枢只负责结构化运行数据；Obsidian 仍保存纪要正文。二者通过 source、refs、双链和更新时间互相指向，不复制全文。</span></div>
        </Panel>
