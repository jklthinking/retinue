...
export default function Sessions({
  ...
  const [taskDept, setTaskDept] = useState("组织中枢");
    ...
    <div className={`rt-page sessions-page ${mobileDetail ? "is-mobile-detail" : ""}`}>
      ...
      <section className="sessions-privacy-note">
        ...
        <div>
          ...
          <p>众卿只接收你授权的层级：默认仅索引；摘要和最近消息需在同步端主动开启。</p>
        </div>
                ...
                {taskOpen && (
                  <form className="session-task-form" onSubmit={(event) => { event.preventDefault(); void createTaskFromSession(); }}>
                    <div className="session-form-grid">
                      ...
                      <label>业务域<input value={taskDept} onChange={(event) => setTaskDept(event.target.value)} placeholder="如：组织中枢、研发、教学" required /></label>
