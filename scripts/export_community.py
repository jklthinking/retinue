...
that pointed at those paths, promotes the community-facing README /
SECURITY / CONTRIBUTING / NOTICE names, and runs three scans: the
identifier and credential rules from ``scripts/check.sh``, plus a
case-insensitive sweep for the internal codename and the Chinese realm
word. The scan report is written next to the export directory, never
inside it.
...
def write_report(
    ...
    lines = [
        ...
        "internal-name sweep is case-insensitive",
        "",
    ...
    lines.append("")
    if identifiers or credentials or internal_names:
...
def export_community(source: Path, dest: Path, report: Path) -> dict[str, object]:
    ...
    write_report(report, identifiers, credentials, internal_names)
    dirty = bool(identifiers or credentials or internal_names)
    return {
        "source": str(source),
        "out": str(dest),
        "report": str(report),
